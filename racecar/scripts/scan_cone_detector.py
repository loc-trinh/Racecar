...
class ConeDetector:
	def __init__(self):
		self.cone_sub = rospy.Subscriber("cone_locator", Float32, self.phi_callback)
		self.cd_sub = rospy.Subscriber("scan", LaserScan, self.laser_callback)
		self.window_angles=Publisher("angeles", Float32, queue_size=4)
		self.cd_pub = Publisher("cone_position", Point32, queue_size=4)
		self.phi = 1000.0
