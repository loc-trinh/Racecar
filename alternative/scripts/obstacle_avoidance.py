...
class ObsticleOvoider:
	...
	def means(self,window):
		...
		return means
	def obs_corners(self,window,start):
		begin=0
		abs_corners=[]
		while begin>len(window)-self.wind:
			m1=np.mean(window[begin:begin+self.wind])
			m2=np.mean(window[begin+self.wind:begin+2*self.wind])
			if abs(m2-m1)>self.corner_thresh:
				abs_corners.append(start+begin+wind)
		return abs_corners
	...
	def obsticleCallback(self,data):
		...
		if min(self.means(window))<obs_threshold:
